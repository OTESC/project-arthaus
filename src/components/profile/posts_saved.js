...
const PostsSaved = () => {
    ...
    const posted_images = [
      giraffe, scenery, scrappers, AI, vr, smiley, scrapper
      ];
...
const clickPosted = () => {
  ...
  setPostedContent ((postedContent === "") ?[postedImages] :"")
}

      const savedImages = <div>
...
const clickSaved = () => {
  ...
  setSavedContent((savedContent === "") ?[savedImages] :"")
}

    return (
        <div className="d-flex pl-2">
            <div><button type="button" className="postsbutton" onClick={clickPosted}>Posts</button>{postedContent}</div>
            <div><button type="button" className="savedbutton" onClick={clickSaved}>Saved </button>{savedContent}</div>
